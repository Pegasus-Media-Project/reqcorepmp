...
useSeoMeta({
  title: 'Timeline — Pegasus Media Project',
...
function getPipelineStatusColors(status: string): { color: string, bg: string, ring: string } {
  const map: Record<string, { color: string, bg: string, ring: string }> = {
    ...
    waitlist: { color: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-50 dark:bg-rose-950/50', ring: 'ring-rose-200 dark:ring-rose-800' },
...
function getStatusBadgeClasses(status: string): string {
  ...
  const map: Record<string, string> = {
    ...
    waitlist: 'bg-rose-100 text-rose-700 dark:bg-rose-900/60 dark:text-rose-300',
