...
import { Sun, Moon, Check, ChevronDown } from 'lucide-vue-next'
import { BILLING_PLANS, getBillingPlan, type BillingPlan, type BillingTier } from '~~/shared/billing'
...
const selectedCadence = computed<BillingCadence>(() => parseBillingCadenceQuery(route.query))
...
const currentPaidPlan = computed(() =>
  selectedIntent.value ? getBillingPlan(selectedIntent.value.planId) ?? null : null,
)
...
const showPlanPicker = computed(() => isSignUp.value || currentPlanId.value !== null)
...
type PlanOption = { id: BillingTier, name: string, tagline: string, price: string, featured: boolean }
const planOptions = computed<PlanOption[]>(() => [
  { id: 'free', name: 'Free trial', tagline: 'Try one role — first AI shortlist free', price: '$0', featured: false },
  ...BILLING_PLANS.map(p => ({ id: p.id, name: p.name, tagline: p.tagline, price: priceLabel(p), featured: p.id === 'team' })),
])
...
const triggerSub = computed(() => {
  if (currentPlanId.value === 'free') return 'Try one role — first AI shortlist free'
  return currentPaidPlan.value?.tagline ?? 'Start with a free trial or pick a paid plan'
})
...
function handlePlanClickOutside(event: MouseEvent) {
  if (planMenuRef.value && !planMenuRef.value.contains(event.target as Node)) {
    planMenuOpen.value = false
  }
}
onMounted(() => document.addEventListener('mousedown', handlePlanClickOutside))
onUnmounted(() => document.removeEventListener('mousedown', handlePlanClickOutside))
  ...
  <div class="relative min-h-screen bg-surface-50 dark:bg-surface-950">
    ...
    >
      ...
      <div class="relative flex items-center gap-3">
        <img src="/eagle-mascot-logo.png" alt="" class="size-9 object-contain" />
        <span class="text-lg font-semibold tracking-tight text-surface-900 dark:text-white">Reqcore</span>
      </div>
      ...
      <div v-if="showPlanPicker" ref="planMenuRef" class="relative mt-12">
        <p class="text-[12px] uppercase tracking-wide text-surface-400 dark:text-white/35">Your plan</p>
        ...
        >
          <span class="min-w-0">
            <span class="block text-[14px] font-semibold text-surface-900 dark:text-white">{{ triggerName }}</span>
            <span class="mt-0.5 block truncate text-[12px] text-surface-400 dark:text-white/40">{{ triggerSub }}</span>
          </span>
          <span class="flex shrink-0 items-center gap-2">
            <span v-if="triggerPrice" class="text-[13px] font-semibold text-surface-700 dark:text-white/80">{{ triggerPrice }}</span>
            <ChevronDown class="size-4 text-surface-400 transition-transform duration-200" :class="planMenuOpen && 'rotate-180'" />
          </span>
        </button>
        ...
        >
          ...
          >
            <!-- Cadence toggle -->
            <div class="grid grid-cols-2 gap-1 border-b border-surface-100 p-1.5 dark:border-white/[0.06]">
              <button
                type="button"
                class="rounded-md px-3 py-1.5 text-[12px] font-medium transition"
                :class="selectedCadence === 'monthly' ? 'bg-surface-100 text-surface-900 dark:bg-white/[0.1] dark:text-white' : 'text-surface-500 hover:text-surface-800 dark:text-white/45 dark:hover:text-white'"
              ...
              <button
                ...
                :class="selectedCadence === 'annual' ? 'bg-surface-100 text-surface-900 dark:bg-white/[0.1] dark:text-white' : 'text-surface-500 hover:text-surface-800 dark:text-white/45 dark:hover:text-white'"
                @click="setCadence('annual')"
              >
                Yearly
                <span class="rounded-full bg-emerald-500/15 px-1.5 py-px text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">2 months free</span>
              </button>
            </div>
            ...
            <ul class="p-1.5">
              <li v-for="opt in planOptions" :key="opt.id">
                <button
                  ...
                  :class="opt.id === currentPlanId && 'bg-surface-50 dark:bg-white/[0.04]'"
                  @click="selectPlan(opt.id)"
                >
                  <span class="flex size-4 shrink-0 items-center justify-center">
                    <Check v-if="opt.id === currentPlanId" class="size-4 text-brand-500" :stroke-width="2.5" />
                  </span>
                  <span class="min-w-0 flex-1">
                    <span class="flex items-center gap-2">
                      <span class="text-[13.5px] font-semibold text-surface-900 dark:text-white">{{ opt.name }}</span>
                      <span v-if="opt.featured" class="rounded-full bg-brand-500/15 px-1.5 py-0.5 text-[10px] font-medium text-brand-600 dark:text-brand-300">Popular</span>
                    </span>
                    <span class="block truncate text-[12px] text-surface-400 dark:text-white/40">{{ opt.tagline }}</span>
                  </span>
                  <span class="shrink-0 text-[13px] font-semibold text-surface-700 dark:text-white/80">{{ opt.price }}</span>
                </button>
              </li>
            </ul>
          </div>
        ...
        <p v-if="currentPaidPlan" class="mt-2 text-[12px] text-surface-400 dark:text-white/35">
    ...
    <div class="relative z-10 flex min-h-screen items-center justify-center overflow-y-auto px-4 py-12 sm:px-8">
      <div class="w-full max-w-[420px]">
        <!-- Compact brand mark — shown when the side panel is hidden -->
        <div class="mb-8 flex flex-col items-center gap-3 text-center xl:hidden">
          <img src="/eagle-mascot-logo.png" alt="Reqcore mascot" class="size-12 object-contain" />
          <span class="text-lg font-semibold tracking-tight text-surface-900 dark:text-surface-100">Reqcore</span>
        </div>
