...
async function seed() {
  ...
  // Seed the org-configurable employment types (the job-form picker list).
  await db.insert(schema.employmentType).values(
    ["Full-time", "Part-time", "Contract", "Internship"].map((label, i) => ({
      id: id(),
      organizationId: orgId,
      label,
      displayOrder: i,
